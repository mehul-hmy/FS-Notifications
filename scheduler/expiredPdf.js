...
const { db, targetDb, bucket } = require("../firebase");
...
const expiredPdf = async () => {
  try {
    ...
    const matchedDoc = notificationMaster.find(
      (doc) => doc?.["notificationName"] === "Document Expired Alert"
    );
...
async function documentRenewalForPdf(accountId, notificationId) {
  ...
  const vehicleMakeSnap = await db
    .collection(`accounts/${accountId}/vehicleMake`)
    .where("isSampleMode", "in", ["Off", "On"])
    .get();
  ...
  scheduleData.forEach((doc) => {
    ...
    if (renewalDate < todayMid) {
      exipredDocs.push(doc);
    }
  });

  let sortedAscData = exipredDocs?.sort(
  ...
  const notificationRecipientSnapshot = await db
    .collection(`accounts/${accountId}/notificationRecipient`)
    .get();
  ...
  // console.log("notificationRecipientRoles", notificationRecipientRoles);

  // 1) Fetch the entire roleUserAssociation sub‑collection
  const roleUserAssocSnapshot = await db
    .collection(`accounts/${accountId}/roleUserAssociation`)
    .get();
...
async function printPdf(data, accountId, upcoming30Data) {
  try {
    ...
    const pdfData = {
      ...
      tempalate: "",
      filterString: "Expired Documents",
      showTotal: false,
...
async function exportAsPDF(data, payload, accountId) {
  try {
    ...
    const fileName = `_${formattedDate}.pdf`;
    // console.log(fileName); // _8 Aug 2025.pdf

    // const fileName = `_${Date.now()}.pdf`; // Include extension
    const destination = `expiredDocPdf/${accountId}/${fileName}`;
    ...
    //   action: "read",
    //   expires: expiryDate, // Set appropriate expiry
    // });
...
cron.schedule("* * * * *", expiredPdf);

module.exports = expiredPdf;
